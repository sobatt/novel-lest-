	...
	<div id="wrap">
		...
		<div id="header">
			...
			<div id="Directory-options">
				<div id="Directory-options-item" v-for="item,index in ulList.slice(0, 8)"" :key="index" @click="changeColor(1,item,item.chapters_id)" :class='{active:index1==item}'>
					Chapter{{item.chapters_id}}
				</div>
	...
	export default {
		...
		data() {
			return {
				...
				pages:"",
				chapter_id: "",
				top:"",img:false,imgs:false,size:false,sizee:false,
				content:"",isftoast:false,nextchap:false,backchap:false,mailfalse:false,nextcpt:true,precpt:true,
				swiperOption: {
					freeMode : true,
					slidesPerView:3
      			}
...
//					console.log(this.chapters2)
				...
				this.$http.post('https://www.meejee.net/chapter/',{book_id:2,chapter_id:2},
				{emulateJSON:true})
				.then(res => {
//				this.list = (res.body.ResultDate.bookInfo);
				this.chapter = (res.body.ResultDate.chapterInfo);
			...
			next:function(){
				...
				this.$http.post('https://www.meejee.net/chapter/',{book_id:2,chapter_id:this.chapter_id},
				{emulateJSON:true}
			).then(res => {
				this.chapter = (res.body.ResultDate.chapterInfo);
...
//					alert('ok');
//					console.log(mail)
					this.$http.post('http://www.meejee.net/subscibe/',{book_id:this.detailId,chapter_id:this.chapter_id,email:mail,type:'facebook'},{emulateJSON:true}
				    ).then(res => {
